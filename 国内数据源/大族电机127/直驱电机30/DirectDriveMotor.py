...
def parse_LMA_page(page_url,headers):
    pro_dic = {}
...
def parse_LMB_page(page_url,headers):
    pro_dic = {}
    tree = etree.HTML(requests.get(url=page_url,headers=headers).text)
...
def parse_LM_page(page_url,headers,series):
    pro_dic = {}
    tree = etree.HTML(requests.get(url=page_url,headers=headers).text)
    pro_basic = parse_LMB_basic(page_url=page_url,tree=tree)
    pro_intro = parse_LMA_intro(page_url=page_url,tree=tree)
...
def parse_LMD_page(page_url,headers,series):
    pro_dic = {}
    tree = etree.HTML(requests.get(url=page_url,headers=headers).text)
    pro_basic = parse_LMB_basic(page_url=page_url,tree=tree)
...
def parse_LMH_page(page_url,headers,series):
    pro_dic = {}
    tree = etree.HTML(requests.get(url=page_url,headers=headers).text)
    pro_basic = parse_LMB_basic(page_url=page_url,tree=tree)
    pro_intro = parse_LMD_intro(page_url=page_url,tree=tree)
...
def parse_series_page(series_kind,page_url,headers):
    if series_kind == 0:
        return parse_LMA_page(page_url=page_url,headers=headers)
    elif series_kind == 1:
        return parse_LMB_page(page_url=page_url,headers=headers)
    elif series_kind == 2:
        return parse_LM_page(page_url=page_url,headers=headers,series=series_kind)
    elif series_kind == 3:
        return parse_LM_page(page_url=page_url,headers=headers,series=series_kind)
    elif series_kind == 4:
        return parse_LM_page(page_url=page_url,headers=headers,series=series_kind)
    elif series_kind == 5:
        return parse_LMH_page(page_url=page_url,headers=headers,series=series_kind)
    elif series_kind == 6:
        return parse_LMD_page(page_url=page_url,headers=headers,series=series_kind)
    return None

def DirectDriveMotor(headers,filename='direct_drive_motor.json',use_proxies=False):
    ...
    for index in range(len(pro_series_urls)):
        ...
        for i in range(length):
            ...
            pro_dic = parse_series_page(series_kind=index,page_url=pro_urls[i],headers=headers)
            final_dic[pro_series_name][pro_names[i]] = pro_dic
...
if __name__ == '__main__':
    ...
    DirectDriveMotor(headers=headers,filename='直驱电机.json',use_proxies=False)
