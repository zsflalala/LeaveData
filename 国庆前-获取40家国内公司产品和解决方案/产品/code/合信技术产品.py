...
def A3S_drive(headers,firstTypeName):
    final_dic = {'pro':[]}
    urls =[
    ...
    for url in urls:
        ...
        Description = {'概览':basic}
        Parameter = {'技术参数':args,'典型应用':application,'安装规格':install_specifications,'样本/手册/软件':sample}

        pro_dic = {}
        pro_dic['ProductName'] = temp_dic['产品名称']
        pro_dic['ProductImage'] = temp_dic['产品图片']
...
def CPU226H_run_controller(url,headers,firstTypeName):
    final_dic = {'pro':[]}
    temp_dic = {}
    resp = requests.get(url=url,headers=headers)
    resp.encoding = 'utf-8'
...
def CTH300_run_ctroller(url,headers,firstTypeName):
    final_dic = {'pro':[]}
    temp_dic = {}
    resp = requests.get(url=url,headers=headers)
    resp.encoding = 'utf-8'
...
def Run_Controller(headers,firstTypeName):
    final_dic = {'pro':[]}
    urls =[
    ...
    final_dic['pro'] += CTH300_run_ctroller(urls[0],headers,firstTypeName)
    final_dic['pro'] += CPU226H_run_controller(urls[1],headers,firstTypeName)
    return  final_dic

def M_motor(headers,firstTypeName):
    final_dic = {'pro':[]}
    temp_dic = {}
...
def Programmable_controller(headers,firstTypeName):
    final_dic = {'pro':[]}
    all_url = [
    ...
    for url in all_url:
        ...
        for i in range(len(urls)):
            ...
            for j in range(len(pro_urls)):
                ...
                pro_dic['ProductJSON'] = ''
                pro_dic['FirstType'] = firstTypeName
                pro_dic['SecondType'] = pro_series
                pro_dic['ProductDetail'] = {}
                pro_dic['ProductDetail']['Description'] = Description
                pro_dic['ProductDetail']['Feature'] = []
                pro_dic['ProductDetail']['Parameter'] = Parameter
                final_dic['pro'].append(pro_dic)
            logger.info(f'已完成{pro_series}系列-->>{title}模块数据爬取')
    return final_dic
...
def COTRUST(headers):
    ...
    if resp.status_code != 200:
        logger.error('状态码: ',resp.status_code)
        return None
    tree = etree.HTML(resp.text)
    
    pro_dic = M_motor(headers=headers,firstTypeName='电机')
    final_dic['pro'] += pro_dic['pro']
    pro_dic = A3S_drive(headers=headers,firstTypeName='驱动器')
    final_dic['pro'] += pro_dic['pro']
    pro_dic = Run_Controller(headers=headers,firstTypeName='运动控制器')
    final_dic['pro'] += pro_dic['pro']
    pro_dic = Programmable_controller(headers=headers,firstTypeName='可编程控制器')
    final_dic['pro'] += pro_dic['pro']
    return final_dic
...
if __name__ == '__main__':
    ...
    final_dic = COTRUST(headers=headers)
    filename=f'合信技术产品{len(final_dic["pro"])}.json'
