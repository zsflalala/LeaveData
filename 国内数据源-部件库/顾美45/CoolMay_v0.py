...
def PLC_EX3GH_spider(product_url,headers):
    tbdata = pd.read_html(product_url,encoding='utf-8')[1]
    ...
    # 返回字典
    return save_normal_table(tbdata=tbdata,tree=tree)

def PLC_spider(headers,filename="plc.json"):
    # PLC一体机主页
    plc_url = 'http://www.coolmay.com/PicList.aspx?ColumnId=58&Language=34&Terminal=41'
    final_dic = {}
    resp = requests.get(url=plc_url,headers=headers)
    tree = etree.HTML(resp.text)
    li_lists = tree.xpath('.//ul[@class="wrapfix"]/li')

    for li in li_lists:
        product_name = li.xpath('./div[2]/a/text()')[0].strip()
        product_url = urljoin(plc_url,li.xpath('./div[2]/a/@href')[0])

        if product_name == 'EX3G-H系列触摸屏PLC一体机':
            product_dic = PLC_EX3GH_spider(product_url=product_url,headers=headers) # 两张表 表格中图片存储数据
        else:
            tbdataframe = pd.read_html(product_url,encoding='utf-8')[0]
            product_dic = save_normal_table(tbdata=tbdataframe)
        final_dic[product_name] = product_dic
...
# 图片类型
def pic_spider(headers,filename='pic.json'):
    # 包含变频器1个 工业开关电源4个 PLC控制器两个(L02 FX3GC各一个)
    url_list = [
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=168&ArticleId=1429&Language=34&Terminal=41',
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=214&ArticleId=2429&Language=34&Terminal=41',
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=214&ArticleId=2430&Language=34&Terminal=41',
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=214&ArticleId=2431&Language=34&Terminal=41',
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=214&ArticleId=2432&Language=34&Terminal=41',
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=211&ArticleId=2486&Language=34&Terminal=41',
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=209&ArticleId=2325&Language=34&Terminal=41',
        'http://www.coolmay.com/ProductDetail.aspx?ColumnId=98&ArticleId=2426&Language=34&Terminal=41'
    ]
    pic_dic = {}
    for url in url_list:
        ...
        img_src = tree.xpath('//samp/img/@src')[0]
        img_url = urljoin(url,img_src)
        pic_dic[product_name] = img_url

    with open(filename, "w", encoding='utf-8') as f:
        # json.dump(dict_, f)  # 写为一行
        json.dump(pic_dic, f, indent=2, sort_keys=False, ensure_ascii=False)  # 写为多行
    print(f"爬取完成，存储在{filename}中.")
...
# CX3G控制器
def PLC_CX3G_control(headers,filename='plc_cx3g.json'):
    cx3g_urls = [
        'http://www.coolmay.com/PicList.aspx?ColumnId=206&Language=34&Terminal=41&page=1',
        'http://www.coolmay.com/PicList.aspx?ColumnId=206&Language=34&Terminal=41&page=2'
    ]
    product_names,product_urls = [],[]
    for url in cx3g_urls:
        resp = requests.get(url=url,headers=headers)
        tree = etree.HTML(resp.text)
        li_list = tree.xpath('.//div[@class="Pro_list"]/ul/li')
        
        for li in li_list:
            product_url = li.xpath('./div[1]/a/@href')[0]
            product_url = urljoin(cx3g_urls[0],product_url)
            product_name = li.xpath('./div[2]/a/text()')[0].strip()
            product_urls.append(product_url)
            product_names.append(product_name)
    for index in range(len(product_urls)):
        resp = requests.get(url=product_urls[index],headers=headers)
        tree = etree.HTML(resp.text)
...
# 保存hmi含有图片的表格
def save_hmi_table(tbdata,url,headers):
    resp = requests.get(url=url,headers=headers)
    tree = etree.HTML(resp.text)
    pro_dic = {}
    ...
    while col < colunm_length:
        ...
        while row < row_length:
            # 含有产品图片的表格
            if tbdata.loc[row][0] == '产  品  图  片':
                img_zheng = tree.xpath(f'//*[@id="tabs_pro"]/div[2]/samp[2]/table/tbody/tr[2]/td[{img_id}]//img/@src')[0]
                img_fan = tree.xpath(f'//*[@id="tabs_pro"]/div[2]/samp[2]/table/tbody/tr[3]/td[{img_id}]//img/@src')[0]
                img_id += 1
                pro_dic[tbdata.loc[0][col]][tbdata.loc[row][0]] = {"正面":img_zheng,"反面":img_fan}
                row += 1
            else:
...
def HMI_spider(headers,filename='hmi.json'):
    hmi_url = 'http://www.coolmay.com/PicList.aspx?ColumnId=66&Language=34&Terminal=41'
    resp = requests.get(url=hmi_url,headers=headers)
    tree = etree.HTML(resp.text)
    ...
    for li in li_lists:
        product_name = li.xpath('./div[2]/a/text()')[0].strip()
        product_url = urljoin(hmi_url,li.xpath('./div[2]/a/@href')[0])
        product_names.append(product_name)
        product_urls.append(product_url)
    for index in range(0,len(product_urls)-1):
        tbdata = pd.read_html(product_urls[index],encoding='utf-8')[0]
        if tbdata.loc[0][0] == tbdata.loc[0][1]:
            pro_dic = save_sensor_table(tbdata=tbdata)
        elif product_names[index] == 'MT60系列真彩触摸屏':
            tbdata.to_csv('temp.csv',mode='w',encoding='utf-8',header=True,index=True)
            pro_dic = save_hmi_table(tbdata=tbdata,url=product_urls[index],headers=headers)
        final_dic[product_names[index]] = pro_dic
...
if __name__ == '__main__':
    ...
    # PLC_spider(headers=headers,filename="plc.json")
    # pic_spider(headers=headers,filename='pic.json')
    # HMI_spider(headers=headers,filename='hmi.json')
    # PLC_CX3G_control(headers=headers,filename='plc_cx3g.json')

    # IOT_spider(headers=headers,filename='iot.json')
    # switch_power_spider(headers=headers,filename='switch_power.json')
    Sensor_spider(headers=headers,filename='sensor.json')
    # frequency_converter(headers=headers,filename='frequence.json')
    run_control_system(headers=headers,filename='run_control_system.json')
