...
def getType(tree,body_html):
    ...
    try:
        span_list = tree.xpath('.//div[@class="breadcrumb"]/span')
        FirstType,SecondType = span_list[1].xpath('./a/text()')[0].strip(),span_list[2].xpath('./a/text()')[0].strip()
        body_html.append(unescape(etree.tostring(tree.xpath('.//div[@class="breadcrumb"]/span')[0],encoding='utf-8').decode()))
    except:
        pass
...
def get_page_source(url,xpath_list,title_xpath,proimg_xpath,needType,needParaFeat):
    ...
    if proimg_xpath != '' and not ProductImage:
        logger.warning(f'ProductImgWarning=>{url}')
...
def getUrls(series_url,urls):
    ...
    a_list = tree.xpath('.//div[@class="col-md-4 tile"]//a')
    for a in a_list:
        urls.append(urljoin(link,a.xpath('./@href')[0]))
    return

if __name__ == '__main__':
    link,source = 'https://www.woodward.com/home','Woodward'
    ...
    url = 'https://www.woodward.com/en/departments/woodward44-actuation'
    resp = requests.get(url=url,headers=headers)
    resp.encoding = 'utf-8'
    tree = etree.HTML(resp.text)
    series_urls = []
    a_list = tree.xpath('.//div[@class="nav-desktop dropdown--mobile"]//ul[@class="accordion--content"]//a')
    for a in a_list:
        series_urls.append(urljoin(link,a.xpath('./@href')[0]))
    series_urls = list(set(series_urls))
    logger.info(f'SeriesUrlsLength {len(series_urls)}')
    ...
    for series_url in series_urls:
        ...
        resp.encoding = 'utf-8'
        tree = etree.HTML(resp.text)
        endpage = int(tree.xpath('.//select[@class="page-index"]/option/text()')[-1].strip())
        for i in range(1,endpage):
            new_series_url = series_url + f'?pg={i}'
            new_series_urls.append(new_series_url)
    series_urls += new_series_urls
    logger.info(f'NewSeriesUrlsLength {len(series_urls)}')
    ...
    title_xpath,proimg_xpath = './/h1[1]/text()','.//div[@class="slider-nav"]//img'
    xpath_list = [
        './/div[@class="intro"]','//section[@class="product-specs-block"]'
    ]
